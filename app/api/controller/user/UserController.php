...
class UserController
{
    ...
    public function user(Request $request)
    {
        ...
        if($user['brokerage'] > $user['extractTotalPrice']) {
            $orderYuePrice = StoreOrder::getOrderStatusYueSum($user['uid']);//余额累计消费
            $systemAdd = UserBill::getSystemAdd($user['uid']);//后台添加余额
            $yueCount = bcadd($user['recharge'],$systemAdd,2);// 后台添加余额 + 累计充值  = 非佣金的总金额
            $orderYuePrice = $yueCount > $orderYuePrice ? 0 : bcsub($orderYuePrice,$yueCount,2);// 余额累计消费（使用佣金消费的金额）
            $user['brokerage'] = bcsub($user['brokerage'],$user['extractTotalPrice'],2);//减去已提现金额
            $extract_price = UserExtract::userExtractTotalPrice($user['uid'],0);
            $user['brokerage'] = $extract_price < $user['brokerage'] ? bcsub($user['brokerage'],$extract_price,2) : 0;//减去审核中的提现金额
            $user['brokerage'] = $user['brokerage'] > $orderYuePrice ? bcsub($user['brokerage'],$orderYuePrice,2) : 0;//减掉余额支付

        }else{
            $user['brokerage']=0;
        }
        $user['extractPrice'] = (float)bcsub($user['brokerage'],$user['extractTotalPrice'],2) > 0 ? : 0;//可提现
        ...
        if(!SystemConfigService::get('vip_open')) $user['vip']=false;//会员开关
